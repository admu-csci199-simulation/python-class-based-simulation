...
def simulationProper(configData, networkData, simulationAgentsList: "list[Agent.Agent]", agenciesList: "list[Agent.Agent]"):
    ...
    hyp5_data = {
        "misinformation": {post["postID"]: [] for post in configData["Posts"] if post["misinformation"]},
        "regular": {post["postID"]: [] for post in configData["Posts"] if not post["misinformation"]}
    }
    ...
    for agent in simulationAgentsList:
        for interaction in agent.interactionsDone:
            post_id = interaction.post.postID
            layer = interaction.layer
            post_type = "misinformation" if interaction.post.isMisinformation else "regular"
            if post_id not in hyp5_data[post_type]:
                continue

            while len(hyp5_data[post_type][post_id]) <= layer:
                hyp5_data[post_type][post_id].append(0)
            hyp5_data[post_type][post_id][layer] += 1
...
def runSimulation(conf_name, output_name):
    ...
    hyp5_data = simulationProper(configData, networkData, agentsList, agenciesList)

    with open(os.path.join("output", output_name), "w") as f:
        json.dump(hyp5_data, f, indent=4)
